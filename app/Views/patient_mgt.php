...
<body>
  <!-- Sidebar and other content goes here... -->
...
<!-- //end -->
  ...
  <div class="myDiv">
    ...
    <div class="input-group mb-3" style="width: 50%; margin: 0 auto;">
      <input type="text" id="searchInput" class="form-control" placeholder="Search by ID or Name" aria-label="Search">
    </div>
    ...
    <table class="table table-info table-hover p-3 text-primary-emphasis bg-primary-subtle border border-primary-subtle rounded-3" style="width: 100%;">
      ...
      <tbody id="patientsTable">
        <?php foreach($ptnt as $patient): ?>
        <tr class="table-light">
          <td><?= $patient['id']?></td>
          <td><?= $patient['name']?></td>
          <td><?= $patient['email']?></td>
          <td><span class="password" id="password-<?= $patient['password'] ?>">**********</span></td>
          <td><?= $patient['phone']?></td>
          <td><?= $patient['address']?></td>
         
          <td> <a href="/editPatient/<?= $patient['id'] ?>" class="btn-theme">Edit</a></td>
